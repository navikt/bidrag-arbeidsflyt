#echo kubectl get pods -o json | jq '.items[].spec.containers[].env[]?.valueFrom.secretKeyRef.name' | grep -v null | sort | uniq | grep bidrag-arbeidsflyt
cd src/test/resources/kafka
SECRET_NAME=aiven-bidrag-arbeidsflyt-XXX
